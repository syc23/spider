...
import copy
import json
from ArticleSpider.items import JdongItem
class DistributeJdSpider(RedisSpider):
    name = 'distribute_jd'
    allowed_domains = ['jd.com','p.3.cn']
    redis_key = 'distribute_jd'
    ...
    def parse(self, response):
        ...
        for dt in dt_list:
            ...
            for em in em_list:
                ss = em.xpath('./a/@href').extract_first()
    ...
    def parse_book_list(self,response):
        ...
        for li in li_list:
            ...
            author = li.xpath('.//span[@class="p-bi-name"]/span/a/@title').extract()
            if len(author) ==0:
                item['book_author'] = ''
            else :
                item['book_author'] = ' '.join(author)
