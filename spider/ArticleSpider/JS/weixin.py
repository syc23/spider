...
class Weixin():
    def __init__(self):
        ...
        self.loginUrl = ' https://mp.weixin.qq.com/cgi-bin/bizlogin?action=startlogin'
    ...
    def getpasswd(self,pw):
        ...
        """
        # 传递js_str,执行js
        js_obj.execute(js_str)
        passwd = js_obj.getPwd(pw)
        return passwd

    def login(self):
        data ={
            'username': 18883245172,
            'pwd': self.getpasswd('a123456'),
            'ajax': '1'
        }
        print(data.get('pwd'))
        html = self.sess.post(self.loginUrl,data=data,headers=self.headers).content.decode('utf-8')
        print(html)
